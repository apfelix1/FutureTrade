...
def get_short_trade_log(future, buyspread, sellspread):
    ...
    curhold = pd.DataFrame(columns=['startDate', 'initAmount', 'canbuy'])
    ...
    for date in tradelist:
        ...
        for index in range(len(curhold.index)):
            if curhold['canbuy'].iloc[index] == False:
        ...
        if curhold['canbuy'].iloc[index] == True:
        ...
        curhold.reset_index(drop=True, inplace=True)
...
if __name__ == '__main__':
    ...
    for i1 in list1:
        for i2 in list2:
            ...
            pathlong = './result/' + name[0:2] + '/rmlongFuture1/'
            pathshort = './result/' + name[0:2] + '/rmshortFuture1/'
